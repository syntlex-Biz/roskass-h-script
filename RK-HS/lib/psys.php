...
//		'EP' =>		array('EgoPay',			'USD', 1, 1, 'USD', 'product_id'),
		...
		'RK' =>     array('Roskassa' ,      'RUB', 1, 1, 'RUB', 'm_operation_id'),
...
//	case 'FK':
//        return array(
//            'acc' => 	array('Account number', '\d{1,6}', '12356')
//        );
	case 'PKPR':
	case 'PKPU':
	case 'PYE':
...
//    case 'FK':
//        return array(
//            'acc' => 		array('Account number', 'F\d{8,10}', 'F123456789'),
//            'id' => 		array('ID', '\d{5,6}', '123456'),
//            'apipass' => 	array('Secret Key')
//        );
	case 'PYE':
	case 'PYR':
...
{
	$c = getCIDs($cid);
	if (($c[4] != 'BTC') and ($c[4] != 'ETH') and ($c[4] != 'XRP') and ($c[4] != 'LTC'))
...
*/
    ...
    case 'RK':
        ...
        ksort($r);
        $str = http_build_query($r);
        $r['sign']= md5($str . $params2['key1']);
        $r['url']="https://pay.roskassa.net/form/?us_paysys=RK&{$str}&sign={$r['sign']}";
        return $r;
	case 'PYE':
	case 'PYR':
	case 'PY':
...
function detectSCI(&$arr)
{
    ...
    if (isset($arr['us_paysys'])) {
            return $arr['us_paysys'];
    }
...
//
	...
	case 'IBC':
		if (!preg_match('|[1-9A-Za-z]{27,34}|', $toparams['acc']))
			return $res;
		$res['answ'] = inet_request("https://block.io/api/v2/withdraw/?api_key=$fromparams[apipass]" .
			//"&from_addresses=$fromparams[acc]" .
			"&to_addresses=$toparams[acc]" .
			"&amounts=$sum".
